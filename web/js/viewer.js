...
function editAliases(button){const personId=Number(button.dataset.personId);const personName=button.dataset.personName;const aliases=JSON.parse(button.dataset.aliases);const groups=JSON.parse(button.dataset.groups||'[]');const box=document.createElement('div');box.className='alias-editor';const note=document.createElement('p');note.textContent='Change the primary name everywhere in LensLedger. Alternate names and groups are optional.';const primaryLabel=document.createElement('label');primaryLabel.textContent='Primary name';const primary=document.createElement('input');primary.value=personName;primary.placeholder='Full name';primaryLabel.append(primary);const aliasLabel=document.createElement('label');aliasLabel.textContent='Other names for this same person (optional — separate each name with a comma)';const aliasInput=document.createElement('input');aliasInput.value=aliases.join(', ');aliasInput.placeholder='Example: Dave, David, Me';aliasLabel.append(aliasInput);const groupLabel=document.createElement('label');groupLabel.textContent='Groups (optional)';const groupTagBox=document.createElement('div');groupTagBox.className='group-tag-box';const selectedGroups=[...groups];let allKnownGroups=[];let highlightedIndex=-1;function renderGroupTags(){groupTagBox.querySelectorAll('.group-tag').forEach(t=>t.remove());selectedGroups.forEach((name,i)=>{const tag=document.createElement('span');tag.className='group-tag';tag.textContent=name;const rm=document.createElement('button');rm.type='button';rm.textContent='×';rm.onclick=e=>{e.stopPropagation();selectedGroups.splice(i,1);renderGroupTags()};tag.append(rm);groupTagBox.insertBefore(tag,groupTagWrapper)})}const groupTagWrapper=document.createElement('div');groupTagWrapper.className='group-tag-wrapper';const groupTagInput=document.createElement('input');groupTagInput.type='text';groupTagInput.className='group-tag-input';groupTagInput.placeholder=groups.length?'Add group…':'e.g. Family, Friends, Band';groupTagInput.autocomplete='off';const groupDropdown=document.createElement('div');groupDropdown.className='group-tag-dropdown';groupTagWrapper.append(groupTagInput,groupDropdown);groupTagBox.append(groupTagWrapper);groupLabel.append(groupTagBox);function filteredOptions(){const q=groupTagInput.value.trim().toLowerCase();return allKnownGroups.filter(g=>!selectedGroups.some(s=>s.toLowerCase()===g.name.toLowerCase())&&(!q||g.name.toLowerCase().includes(q)))}function renderDropdown(){const opts=filteredOptions();const q=groupTagInput.value.trim();groupDropdown.replaceChildren();if(!opts.length&&!q){groupDropdown.classList.remove('open');return}opts.forEach((g,i)=>{const btn=document.createElement('button');btn.type='button';btn.className='group-tag-option';if(i===highlightedIndex)btn.classList.add('highlighted');btn.innerHTML='';btn.append(document.createTextNode(g.name));const count=document.createElement('small');count.textContent='('+g.member_count+')';btn.append(count);btn.onmouseenter=()=>{highlightedIndex=i;renderDropdown()};btn.onclick=e=>{e.stopPropagation();addGroup(g.name)};groupDropdown.append(btn)});if(q&&!opts.some(g=>g.name.toLowerCase()===q.toLowerCase())&&!selectedGroups.some(s=>s.toLowerCase()===q.toLowerCase())){const newBtn=document.createElement('button');newBtn.type='button';newBtn.className='group-tag-option';newBtn.textContent='+ Create "'+q+'"';const idx=opts.length;if(highlightedIndex===idx)newBtn.classList.add('highlighted');newBtn.onmouseenter=()=>{highlightedIndex=idx;renderDropdown()};newBtn.onclick=e=>{e.stopPropagation();addGroup(q)};groupDropdown.append(newBtn)}groupDropdown.classList.add('open');if(highlightedIndex>=groupDropdown.children.length)highlightedIndex=groupDropdown.children.length-1}function addGroup(name){const trimmed=name.trim();if(!trimmed)return;if(!selectedGroups.some(s=>s.toLowerCase()===trimmed.toLowerCase())){selectedGroups.push(trimmed)}groupTagInput.value='';highlightedIndex=-1;renderGroupTags();renderDropdown();groupTagInput.placeholder='Add group…';groupTagInput.focus()}groupTagInput.oninput=()=>{highlightedIndex=0;renderDropdown()};groupTagInput.onfocus=()=>{highlightedIndex=-1;renderDropdown()};groupTagInput.onkeydown=e=>{const totalItems=groupDropdown.querySelectorAll('.group-tag-option').length;if(e.key==='ArrowDown'){e.preventDefault();highlightedIndex=Math.min(highlightedIndex+1,totalItems-1);renderDropdown()}else if(e.key==='ArrowUp'){e.preventDefault();highlightedIndex=Math.max(highlightedIndex-1,0);renderDropdown()}else if(e.key==='Enter'){e.preventDefault();const items=groupDropdown.querySelectorAll('.group-tag-option');if(highlightedIndex>=0&&highlightedIndex<items.length){items[highlightedIndex].click()}else{const q=groupTagInput.value.trim();if(q)addGroup(q)}}else if(e.key==='Escape'){groupDropdown.classList.remove('open')}else if(e.key==='Backspace'&&!groupTagInput.value&&selectedGroups.length){selectedGroups.pop();renderGroupTags();renderDropdown()}};groupTagBox.onclick=()=>groupTagInput.focus();document.addEventListener('click',function closeGroupDrop(e){if(!groupTagBox.contains(e.target)){groupDropdown.classList.remove('open')}});renderGroupTags();fetch('/api/groups').then(r=>r.json()).then(data=>{allKnownGroups=data.groups||[];renderDropdown()}).catch(()=>{});const actions=document.createElement('div');actions.className='publish-actions';const cancel=document.createElement('button');cancel.className='secondary';cancel.textContent='Cancel';cancel.onclick=closeModal;const save=document.createElement('button');save.textContent='Save';save.onclick=async()=>{save.disabled=true;try{await api('/api/person/names',{person_id:personId,name:primary.value,aliases:aliasInput.value.split(',')});await api('/api/person/groups',{person_id:personId,groups:selectedGroups});location.reload()}catch(e){save.disabled=false;note.textContent=e.message;note.classList.add('status-error')}};actions.append(cancel,save);box.append(note,primaryLabel,aliasLabel,groupLabel,actions);openModal('Edit '+personName,box);primary.focus();primary.select()}
