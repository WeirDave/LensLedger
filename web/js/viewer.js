...
function editAliases(button){const personId=Number(button.dataset.personId);const personName=button.dataset.personName;const aliases=JSON.parse(button.dataset.aliases);const groups=JSON.parse(button.dataset.groups||'[]');const box=document.createElement('div');box.className='alias-editor';const note=document.createElement('p');note.textContent='Change the primary name everywhere in LensLedger. Alternate names and groups are optional.';const primaryLabel=document.createElement('label');primaryLabel.textContent='Primary name';const primary=document.createElement('input');primary.value=personName;primary.placeholder='Full name';primaryLabel.append(primary);const aliasLabel=document.createElement('label');aliasLabel.textContent='Other names for this same person (optional — separate each name with a comma)';const aliasInput=document.createElement('input');aliasInput.value=aliases.join(', ');aliasInput.placeholder='Example: Dave, David, Me';aliasLabel.append(aliasInput);const groupSection=document.createElement('div');groupSection.className='group-tag-section';const groupLabel=document.createElement('label');groupLabel.textContent='Groups (optional)';groupLabel.className='group-tag-label';const groupContainer=document.createElement('div');groupContainer.className='group-tag-container';const groupTagBox=document.createElement('div');groupTagBox.className='group-tag-box';const groupTagInput=document.createElement('input');groupTagInput.type='text';groupTagInput.className='group-tag-input';groupTagInput.placeholder=groups.length?'Add group…':'e.g. Family, Friends, Band';groupTagInput.autocomplete='off';const groupDropdown=document.createElement('div');groupDropdown.className='group-tag-dropdown';groupTagBox.append(groupTagInput);groupContainer.append(groupTagBox,groupDropdown);groupSection.append(groupLabel,groupContainer);const selectedGroups=[...groups];let allKnownGroups=[];let highlightedIndex=-1;let dropdownOpen=false;function renderGroupTags(){groupTagBox.querySelectorAll('.group-tag').forEach(t=>t.remove());selectedGroups.forEach((name,i)=>{const tag=document.createElement('span');tag.className='group-tag';tag.textContent=name;const rm=document.createElement('button');rm.type='button';rm.textContent='×';rm.onclick=e=>{e.stopPropagation();selectedGroups.splice(i,1);renderGroupTags();if(dropdownOpen)renderDropdown()};tag.append(rm);groupTagBox.insertBefore(tag,groupTagInput)})}function filteredOptions(){const q=groupTagInput.value.trim().toLowerCase();return allKnownGroups.filter(g=>!selectedGroups.some(s=>s.toLowerCase()===g.name.toLowerCase())&&(!q||g.name.toLowerCase().includes(q)))}function showDropdown(){dropdownOpen=true;renderDropdown()}function hideDropdown(){dropdownOpen=false;highlightedIndex=-1;groupDropdown.classList.remove('open');groupDropdown.replaceChildren()}function renderDropdown(){if(!dropdownOpen)return;const opts=filteredOptions();const q=groupTagInput.value.trim();groupDropdown.replaceChildren();if(!opts.length&&!q){groupDropdown.classList.remove('open');return}opts.forEach((g,i)=>{const btn=document.createElement('button');btn.type='button';btn.className='group-tag-option';if(i===highlightedIndex)btn.classList.add('highlighted');btn.append(document.createTextNode(g.name+' '));const count=document.createElement('small');count.textContent='('+g.member_count+')';btn.append(count);btn.onmousemove=()=>{if(highlightedIndex!==i){groupDropdown.querySelector('.highlighted')?.classList.remove('highlighted');highlightedIndex=i;btn.classList.add('highlighted')}};btn.onmousedown=e=>e.preventDefault();btn.onclick=e=>{e.stopPropagation();addGroup(g.name)};groupDropdown.append(btn)});if(q&&!opts.some(g=>g.name.toLowerCase()===q.toLowerCase())&&!selectedGroups.some(s=>s.toLowerCase()===q.toLowerCase())){const newBtn=document.createElement('button');newBtn.type='button';newBtn.className='group-tag-option';newBtn.textContent='+ Create "'+q+'"';const idx=opts.length;if(highlightedIndex===idx)newBtn.classList.add('highlighted');newBtn.onmousemove=()=>{if(highlightedIndex!==idx){groupDropdown.querySelector('.highlighted')?.classList.remove('highlighted');highlightedIndex=idx;newBtn.classList.add('highlighted')}};newBtn.onmousedown=e=>e.preventDefault();newBtn.onclick=e=>{e.stopPropagation();addGroup(q)};groupDropdown.append(newBtn)}groupDropdown.classList.add('open');if(highlightedIndex>=groupDropdown.children.length)highlightedIndex=groupDropdown.children.length-1}function addGroup(name){const trimmed=name.trim();if(!trimmed)return;if(!selectedGroups.some(s=>s.toLowerCase()===trimmed.toLowerCase())){selectedGroups.push(trimmed)}groupTagInput.value='';highlightedIndex=-1;renderGroupTags();hideDropdown();groupTagInput.placeholder='Add group…';groupTagInput.focus()}groupTagInput.oninput=()=>{highlightedIndex=0;showDropdown()};groupTagInput.onkeydown=e=>{if(e.key==='ArrowDown'){e.preventDefault();if(!dropdownOpen){highlightedIndex=0;showDropdown()}else{const total=groupDropdown.children.length;highlightedIndex=Math.min(highlightedIndex+1,total-1);renderDropdown()}}else if(e.key==='ArrowUp'){e.preventDefault();highlightedIndex=Math.max(highlightedIndex-1,0);renderDropdown()}else if(e.key==='Enter'){e.preventDefault();if(dropdownOpen&&highlightedIndex>=0&&highlightedIndex<groupDropdown.children.length){groupDropdown.children[highlightedIndex].click()}else{const q=groupTagInput.value.trim();if(q)addGroup(q)}}else if(e.key==='Backspace'&&!groupTagInput.value&&selectedGroups.length){selectedGroups.pop();renderGroupTags();if(dropdownOpen)renderDropdown()}else if(e.key==='Tab'){hideDropdown()}};groupTagBox.addEventListener('click',e=>{if(e.target.closest('.group-tag'))return;groupTagInput.focus();if(!dropdownOpen){highlightedIndex=-1;showDropdown()}});renderGroupTags();groupDropdown.addEventListener('group-dropdown-close',hideDropdown);document.addEventListener('mousedown',e=>{if(!groupContainer.contains(e.target))hideDropdown()});fetch('/api/groups').then(r=>r.json()).then(data=>{allKnownGroups=data.groups||[]}).catch(()=>{});const actions=document.createElement('div');actions.className='publish-actions';const cancel=document.createElement('button');cancel.className='secondary';cancel.textContent='Cancel';cancel.onclick=closeModal;const save=document.createElement('button');save.textContent='Save';save.onclick=async()=>{save.disabled=true;try{await api('/api/person/names',{person_id:personId,name:primary.value,aliases:aliasInput.value.split(',')});await api('/api/person/groups',{person_id:personId,groups:selectedGroups});location.reload()}catch(e){save.disabled=false;note.textContent=e.message;note.classList.add('status-error')}};actions.append(cancel,save);box.append(note,primaryLabel,aliasLabel,groupSection,actions);openModal('Edit '+personName,box);primary.focus();primary.select()}
...
async function openUpdatePanel(){
 ...
 async function refresh(){try{const status=await fetch('/api/update/status').then(response=>response.json());if(!status.managed_install){const sourceCheckout=status.is_source_checkout;const canGit=!!status.can_git_update;install.hidden=true;if(sourceCheckout&&status.restart_ready){title.textContent='LensLedger '+status.on_disk_version+' is on disk, not running yet';message.textContent='This process is still running '+status.current_version+'. The code in this folder has already moved on since it started — restart to pick it up.';location.textContent='This copy: '+status.current_install_root;restart.hidden=false;restart.disabled=false;return}restart.hidden=true;if(status.state==='checking'){title.textContent='Checking for updates…';message.textContent='Contacting GitHub for the latest verified release.';location.textContent='This copy: '+status.current_install_root;setTimeout(refresh,500);return}title.textContent=status.state==='available'?'LensLedger '+status.release.version+' is available':'Update check';message.textContent=status.state==='available'?(canGit?"Update now to fetch and check out the newest release in this folder.":(sourceCheckout?"This is a source checkout, so it can't update itself automatically.":"This copy was run directly from an extracted download, so it can't update itself automatically.")):(status.message||status.state);if(canGit&&status.state==='available'){install.hidden=false;install.disabled=false;}location.textContent=sourceCheckout?('This copy: '+status.current_install_root+(canGit?'. Updates check out the newest release tag here.':'. Update it with git pull, or run Install LensLedger.cmd once to create a separate managed copy.')):('This copy: '+status.current_install_root+'. Run Install LensLedger.cmd once from this same folder to move LensLedger into a managed copy that updates itself automatically from then on — your photo library and catalog are unaffected.');return}restart.hidden=true;message.textContent=status.message||status.state;location.textContent='Managed installation: '+status.current_install_root;if(status.state==='checking'){title.textContent='Checking for updates…';install.hidden=true;setTimeout(refresh,500);return}if(status.state==='available'){title.textContent='LensLedger '+status.release.version+' is available';install.hidden=false;install.disabled=false;return}if(status.state==='current'){title.textContent='LensLedger '+status.current_version+' is current';install.hidden=true;return}if(status.state==='restarting'){title.textContent='Installing and restarting…';install.hidden=true;check.disabled=true;waitForRestart();return}title.textContent='Update check needs attention';install.hidden=true}catch(error){title.textContent='Update check needs attention';message.textContent=error.message}}
