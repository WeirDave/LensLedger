...
async function openMenuPanel(name){closeMenu();if(name==='library')return openLibraryPanelV2();if(name==='update')return openUpdatePanel();if(name==='guide'){const box=document.createElement('div');['Use Primary subject for the main thing in one photo, Photo tags for other visible things, People for confirmed identities, and Event / folder tags for context shared by the whole batch. Search uses Everything by default so all four contribute to normal results.','Face matches are suggestions until you confirm them. Open Capture details to see readable EXIF, IPTC, and XMP details already stored in the file.','To identify people: go to Name faces and name a handful of different people (5–10 is plenty), then go to Review people where LensLedger uses what you taught it to suggest matches across your entire library. You don’t need to name every face by hand — just seed the system and let it do the work.'].forEach(t=>{const p=document.createElement('p');p.textContent=t;box.append(p)});const kh=document.createElement('h3');kh.textContent='Keyboard & mouse shortcuts';kh.style.margin='18px 0 8px';box.append(kh);const kt=document.createElement('table');kt.className='guide-formats';kt.innerHTML='<thead><tr><th>Action</th><th>Shortcut</th></tr></thead><tbody>'+'<tr><td>Next / previous photo</td><td>← → arrow keys</td></tr>'+'<tr><td>Open photo in file explorer</td><td>Double-click the main image</td></tr>'+'<tr><td>Toggle 3× zoom</td><td>Triple-click the main image</td></tr>'+'<tr><td>Pan while zoomed</td><td>Click and drag</td></tr>'+'<tr><td>Select a photo for batch editing</td><td>Ctrl+click (or ⌘+click) a thumbnail</td></tr>'+'<tr><td>Select a range of photos</td><td>Shift+click a thumbnail</td></tr>'+'<tr><td>Close any panel or dialog</td><td>Escape</td></tr>'+'</tbody>';box.append(kt);const lh=document.createElement('h3');lh.textContent='Library management';lh.style.margin='18px 0 8px';box.append(lh);const lbox=document.createElement('div');lbox.style.cssText='font-size:0.92em;line-height:1.5';['<b>How libraries work</b> — A library is one root folder. Everything inside it (all subfolders, any depth) belongs to that library. Each library has its own separate database — there is no shared master database.','<b>Add a library</b> — Go to Settings and click "Add library." The database is stored in a hidden .LensLedger folder inside your photo library by default, so the index travels with your photos.','<b>Switch libraries</b> — Click "Switch" next to any library in the Settings list.','<b>Relocate a library</b> — If you moved your photos to a new folder (e.g. from a USB drive to your hard drive), click "Relocate" next to the library in Settings and choose the new location. Your tags, scan results, and index carry over.'].forEach(t=>{const p=document.createElement('p');p.style.margin='4px 0';p.innerHTML=t;lbox.append(p)});box.append(lbox);const h=document.createElement('h3');h.textContent='Supported image formats';h.style.margin='18px 0 8px';box.append(h);const note=document.createElement('p');note.style.cssText='font-size:0.92em;color:var(--text-secondary);margin:0 0 8px';note.textContent='Publishable means LensLedger can write people names and tags back into the file’s metadata.';box.append(note);const tbl=document.createElement('table');tbl.className='guide-formats';tbl.innerHTML='<thead><tr><th>Format</th><th>Metadata</th><th>Faces</th><th>Viewable</th><th>Publishable</th></tr></thead><tbody>'+'<tr><td>JPEG (.jpg, .jpeg)</td><td>✓</td><td>✓</td><td>✓</td><td>✓</td></tr>'+'<tr><td>PNG (.png)</td><td>✓</td><td>✓</td><td>✓</td><td>—</td></tr>'+'<tr><td>WebP (.webp)</td><td>✓</td><td>✓</td><td>✓</td><td>—</td></tr>'+'<tr><td>TIFF (.tif, .tiff)</td><td>✓</td><td>✓</td><td>✓</td><td>—</td></tr>'+'<tr><td>GIF (.gif)</td><td>—</td><td>✓</td><td>✓</td><td>—</td></tr>'+'<tr><td>BMP (.bmp)</td><td>—</td><td>✓</td><td>✓</td><td>—</td></tr>'+'<tr><td>HEIC / HEIF</td><td>—</td><td>✓</td><td>✓ *</td><td>✓</td></tr>'+'<tr><td>RAW (.dng, .cr2, .cr3, .nef, .arw, .orf, .rw2, .raf)</td><td>—</td><td>—</td><td>—</td><td>—</td></tr>'+'<tr><td>Video (.mp4, .mov, .avi, .wmv, .mpg, .mpeg, .mkv)</td><td>—</td><td>—</td><td>—</td><td>—</td></tr>'+'</tbody>';box.append(tbl);const foot=document.createElement('p');foot.style.cssText='font-size:0.85em;color:var(--text-secondary);margin:6px 0 0';foot.textContent='* HEIC/HEIF files are converted to JPEG on the fly for viewing. RAW and video files are indexed and searchable but not viewable or face-scanned.';box.append(foot);return openModal('Quick guide',box)};if(name==='about'){const o=document.getElementById('aboutOverlay');if(o){o.classList.add('open');document.getElementById('aboutClose').onclick=document.getElementById('aboutOverlay').onclick=function(e){if(e.target===this||e.target.id==='aboutClose')document.getElementById('aboutOverlay').classList.remove('open')}}return};if(name==='trash')return openTrashPanel()}
