...
async function refresh() {
  try {
    const [diagnostics, location, ocr, semantic, faceScan] = await Promise.all([
    ...
    $('healthSummary').replaceChildren(
      ...
      metric('Mapped photos', c.mapped, c.mapped ? () => { location.href = '/map'; } : null),
      metric('People to review', c.people_pending, c.people_pending ? () => { location.href = '/people-review'; } : null),
    ...
    const locationScanning = location.state === 'scanning';
    $('locationMessage').textContent = location.message || 'Not run yet — click below to find GPS coordinates in your photos.';
    $('locationMetrics').replaceChildren(
      metric('Scanned', location.scanned), metric('Changed', location.changed),
      metric('Unchanged', location.unchanged), metric('Errors', location.errors),
    );
    $('startLocation').disabled = locationScanning;
    $('pauseLocation').disabled = !locationScanning;
    setSpinner('locationSpinner', locationScanning);
    $('locationElapsed').textContent = locationScanning ? elapsedText(location.started_at) : '';
    ...
    $('startOcr').disabled = ocrRunning;
    $('pauseOcr').disabled = !ocrRunning;
    ...
    if (!semantic.installed) {
      ...
      $('installSemantic').disabled = semanticInstalling;
    ...
    $('startSemantic').disabled = semanticRunning;
    $('pauseSemantic').disabled = !semanticRunning;
    ...
    if (!faceScan.installed) {
      ...
      $('installFaceScan').disabled = faceInstalling;
    ...
    $('startFaceScan').disabled = faceScanRunning;
    $('pauseFaceScan').disabled = !faceScanRunning;
    ...
    const anyActive = locationScanning || ocrRunning || semanticRunning || semanticInstalling || faceScanRunning || faceInstalling;
