...
.lightbox{display:none;position:fixed;z-index:20;inset:0;background:#05070bef;padding:24px;grid-template-rows:auto minmax(0,1fr)}.lightbox.open{display:grid}.lightbox-head{display:flex;justify-content:flex-end;margin-bottom:10px}.lightbox-photo{position:relative;min-height:0}.lightbox-photo>img{position:absolute;inset:0;width:100%;height:100%;object-fit:contain;min-height:0}
...
function hasFaceBox(item){return [item.box_left,item.box_top,item.box_right,item.box_bottom].every(value=>Number.isFinite(value))}
function markFace(container,img,item){container.querySelectorAll('.face-box,.location-note').forEach(node=>node.remove());if(!hasFaceBox(item)){const note=document.createElement('span');note.className='location-note';note.textContent='Exact face location not recovered yet';container.append(note);return}const marker=document.createElement('div');marker.className='face-box';marker.innerHTML='<span>Face being checked</span>';container.append(marker);const position=()=>{if(!img.naturalWidth||!img.naturalHeight)return;const scale=Math.min(img.clientWidth/img.naturalWidth,img.clientHeight/img.naturalHeight);const shownWidth=img.naturalWidth*scale,shownHeight=img.naturalHeight*scale;const offsetX=(img.clientWidth-shownWidth)/2,offsetY=(img.clientHeight-shownHeight)/2;const left=offsetX+item.box_left*shownWidth,top=offsetY+item.box_top*shownHeight;marker.style.left=left+'px';marker.style.top=top+'px';marker.style.width=((item.box_right-item.box_left)*shownWidth)+'px';marker.style.height=((item.box_bottom-item.box_top)*shownHeight)+'px';marker.classList.toggle('label-right',left+130>container.clientWidth);marker.classList.toggle('label-below',top<34)};img.addEventListener('load',position,{once:true});if(img.complete)position();new ResizeObserver(position).observe(container)}
function buildCard(item){const card=document.createElement('article');card.className='review-card';card.dataset.id=item.id;card.innerHTML='<div class="photo-box"><img loading="lazy" alt="Suggested photo"><span class="state-badge">✓ Contains person</span><button type="button" class="expand" title="Show the full photo larger">⛶ Enlarge</button></div><div class="card-info"><div class="file-line"><div><strong></strong><small></small></div><span class="confidence"></span></div><button type="button" class="toggle-wrong">This photo contains '+escapeText(queue.person.name)+'</button><div class="correction"><label>If you know who it is, enter the correct name (optional)</label><input list="peopleOptions" placeholder="Correct name"></div></div>';const img=card.querySelector('img');img.src='/media?id='+item.id;markFace(card.querySelector('.photo-box'),img,item);card.querySelector('strong').textContent=item.filename;card.querySelector('small').textContent=(item.capture_date||'Date unknown')+' · '+item.folder;card.querySelector('.confidence').textContent=Math.round((item.confidence||0)*100)+'%';card.querySelector('.toggle-wrong').onclick=()=>toggleCard(card,item);card.querySelector('.expand').onclick=()=>openLarge(item);const input=card.querySelector('input');input.oninput=()=>corrections.set(item.id,input.value.trim());input.onclick=e=>e.stopPropagation();return card}
...
function openLarge(item){$('largePhoto').src='/media?id='+item.id;markFace($('largePhotoBox'),$('largePhoto'),item,false);$('lightbox').classList.add('open')}
function closeLarge(){$('lightbox').classList.remove('open');$('largePhoto').removeAttribute('src');$('largePhotoBox').querySelectorAll('.face-box,.location-note').forEach(node=>node.remove())}
