...
def load_library_state() -> Path:
    try:
        value = json.loads(LIBRARY_STATE_PATH.read_text(encoding="utf-8")).get("root", "")
...
def save_library_state(root: Path) -> None:
    ...
    temporary.write_text(json.dumps({"root": str(root)}, indent=2), encoding="utf-8")
...
{people_gallery_html}{people_result_bar}<main class="viewer"{" style=\"display:none\"" if gallery_mode else ""}><section class="upper"><div class="stage" id="stage"><div class="empty">Choose a photo from the filmstrip</div><button class="stage-nav" id="previousPhoto">‹</button><button class="stage-nav" id="nextPhoto">›</button></div><aside class="sidebar">
    ...
    def open_library(self, body):
        ...
        with type(self).library_lock:
            ...
            type(self).library_job = {
                "state": "scanning", "message": f"Indexing {root}", "target_root": str(root),
            }
        handler_class = type(self)

        def worker():
            try:
                result = scan_library(root, database)
                ...
                with handler_class.library_lock:
                    handler_class.library_job = {
                        "state": "complete", "message": f"Opened {root}", "target_root": str(root),
                    }
            except Exception as exc:
                with handler_class.library_lock:
                    handler_class.library_job = {
