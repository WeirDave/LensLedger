...
def nav_menu(current_page: str = "", library_root: str = "") -> str:
    ...
    about_modal = (
        ...
        f'<img src="/logo.png?v={APP_VERSION}" alt="LensLedger" class="about-logo">'
        f'<h2 class="about-title">{APP_NAME}</h2>'
        f'<p class="about-tagline">{APP_TAGLINE}</p>'
        f'<span class="about-version">v{APP_VERSION}</span>'
        '</div>'
        '<div class="about-body">'
        + (f'<div class="about-library"><span class="about-library-label">Current library</span>'
           f'<span class="about-library-path">{html.escape(library_root)}</span></div>'
           if library_root else "")
        ...
        '<p class="about-section-label">Other projects by the developer</p>'
        '<div class="about-projects">'
        '<a href="https://github.com/WeirDave/WaxFrame-Professional" target="_blank" rel="noopener" class="about-project">'
        '<strong>WaxFrame Professional</strong><span>Multi-AI document refinement in your browser.</span></a>'
        '<a href="https://github.com/WeirDave/WaxFrame-Free" target="_blank" rel="noopener" class="about-project">'
        '<strong>WaxFrame Free</strong><span>Standalone browser-based free version. No install required.</span></a>'
        '<a href="https://github.com/WeirDave/Subscription-Wizard" target="_blank" rel="noopener" class="about-project">'
        '<strong>Subscription Wizard</strong><span>Firefox extension to compare and manage Amazon Subscribe &amp; Save subscriptions.</span></a>'
        '<a href="https://github.com/WeirDave/WD-Wireless-Tools" target="_blank" rel="noopener" class="about-project">'
        '<strong>WD Wireless Tools</strong><span>Ekahau workflow tools for wireless network professionals.</span></a>'
        '</div>'
        '</div></div></div>'
    )
...
<div class="reconnect-actions"><div class="picker-prefs"><label class="toggle-row"><input type="checkbox" id="alwaysAsk" checked> Ask which library to open at startup</label></div><div class="actions"><span class="spacer"></span><button type="button" id="openSelected">Open library</button></div></div>
    ...
    def map_page(self):
        page = f"""<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Photo map — {APP_NAME}</title><link rel="icon" href="/favicon.png?v={APP_VERSION}"><link rel="stylesheet" href="{asset_url('css/theme.css')}"><link rel="stylesheet" href="{asset_url('css/leaflet.css')}"><link rel="stylesheet" href="{asset_url('css/map.css')}">
...
<main class="map-shell" id="viewport"><div id="markerLayer"></div><div id="streetMap" hidden></div><div class="controls"><button type="button" id="zoomIn" aria-label="Zoom in">+</button><button type="button" id="zoomOut" aria-label="Zoom out">−</button><button type="button" id="reset" aria-label="Reset map">⌂</button></div><div class="legend"><strong>Photo locations</strong>Drag to rotate and scroll to zoom. Nearby coordinates are grouped; select a marker to browse every photo from that place.<small class="map-credit">Coastlines © <a href="https://commons.wikimedia.org/wiki/User:Tubs" target="_blank" rel="noopener">TUBS</a>, Wikimedia Commons (CC BY-SA 3.0)</small></div><aside class="details" id="details"><img id="preview" alt="Representative photo from this location"><div class="details-body"><h2 id="placeTitle"></h2><p id="placeDates"></p><p id="placeCoords"></p><div class="details-actions"><a class="button" id="openPhoto">Open photo</a><a class="button secondary" id="viewAllHere">View all photos here</a><a class="button secondary" id="openStreetMap" target="_blank" rel="noopener">Open in OpenStreetMap ↗</a><button type="button" id="closeDetails">Close</button></div></div></aside><div class="lightbox" id="lightbox"><button type="button" class="lightbox-close" id="lightboxClose" aria-label="Close">×</button><button type="button" class="lightbox-prev" id="lightboxPrev" aria-label="Previous photo">‹</button><button type="button" class="lightbox-next" id="lightboxNext" aria-label="Next photo">›</button><div class="lightbox-body"><img id="lightboxImg" alt="Photo preview"><div class="lightbox-info"><h2 id="lightboxTitle"></h2><p id="lightboxDate"></p><p id="lightboxCoords"></p><div class="lightbox-counter" id="lightboxCounter"></div><div class="lightbox-actions"><a class="button secondary" id="lightboxOpen">Open in viewer</a><a class="button secondary" id="lightboxAll">View all photos here</a><a class="button secondary" id="lightboxOsm" target="_blank" rel="noopener">OpenStreetMap ↗</a></div></div></div></div><section class="empty" id="empty"><div><h2>No mapped photos yet</h2><p id="emptyText">Run an incremental library scan to collect embedded GPS coordinates. LensLedger reads them locally and never writes location data back to your files.</p><a class="button" href="/scan-photos">Scan your photos</a></div></section></main>
<script src="{asset_url('js/three-min.js')}"></script>
...
<section class="card" id="face-detection"><h2>Face detection model</h2><div id="faceSetupArea"><p id="faceSetupMsg">Checking face detection status…</p><div class="progress-bar" id="faceInstallBarWrap" hidden><span id="faceInstallBar"></span></div><div class="semantic-install-actions" id="faceInstallActions"><button type="button" id="installFaceBtn">Set up face detection</button></div></div><div id="faceModelInfo" hidden><p class="model-intro">Face detection uses the InsightFace buffalo_l model for local face recognition. The model runs entirely on this computer; nothing is ever uploaded.</p><div class="model-list" id="faceModelList"></div></div></section>
...
<script src="{asset_url('js/publish.js')}" defer></script>
    ...
    def _undo_people_review_action(self, con, action_id):
        ...
        if disposition_column and previous["face_id"] is not None:
            if disposition_column not in {"ignored_at", "unknown_at"}:
                raise ValueError("invalid disposition column")
            con.execute(
...
<script src="{asset_url('js/group-manager.js')}" defer></script>
    ...
    def recent_log(self):
        ...
        if log_file.is_file():
            try:
                ...
                console_log(f"Could not read log file: {exc}")
    ...
    def face_scan_job_status(self):
        ...
        cache = face_model_cache_info() if coverage.get("installed") else {}
    ...
    def start_face_scan(self, _body):
        with type(self).face_scan_lock:
            ...
            if not face_is_available():
                raise ValueError(
                    'Face detection is not set up yet. Install it from the Settings page first.'
                )
            with type(self).library_lock:
                if type(self).library_job.get("state") == "scanning":
